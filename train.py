...
def train(dataloader, model, optimizer, criterion, criterion2, device, is_topk, class_weights, scoring_mode):
	with torch.set_grad_enabled(True):
		...
		for i, (input, label) in enumerate(pbar):
			try:
				...
				croloss = CENTROPY(logits, logits2, seq_len, device, scoring_mode, sample_weights)
